...
pass an object or string as the 3rd argument to CleverscriptInput.
...
-debug: pass this in to see an alert with all the variables, 0=none, 1=data, 2=data+options, 3=request, 4=show POST form/iframe, 5=force POST (0)
...
function CleverscriptSetup (apikey, options) { //added 1/8/2013 to separate form from options
    ...
    if (!options.extra_variables) options.extra_variables = ''; //any extra variables to be passed in, now passed directly into CleverscriptInput
    options.history = new Array(); //history of interactions
    options.tts_url = ''; //the waiting URL for TTS
...
function ShowCleverscriptForm (apikey, options) {
    ...
    if (options.bot_start=='yes') CleverscriptInput ('', botnum, options.extra_variables); //make the bot start the conversation, pass in extra variables
}
...
function CleverscriptInput (inputel, botnum, extravars) {
    if (!botnum) botnum = 0;
    var options = CSBOTS[botnum];
    ...
    var url = ''; //protocol added below so that it could be https
    if (options.server=='test') url += 'http://testapi.cleverscript.com/csapi'; //test server
    else if (options.server=='live') url += 'http://api.cleverscript.com/csapi'; //live server
    else url += options.server; //use server that was passed in
    if (url.indexOf ('http') < 0) url = 'http://' + url; //prefix by http:// if it doesn't already have it
    //url += '/csapi'; //added directly to test/live so that it doesn't have to be part of a passed in server
    ...
    rex = ''; //extra variables
    if (typeof extravars == 'object') for (var p in extravars) rex += p + '=' + encodeURIComponent (vars[p]) + '&';
    else if (typeof extravars == 'string') rex = extravars;
    else if (typeof extravars == 'undefined') rex = '';
    geturl += rex; //parse the extra variables here from 7/11/2014
    //geturl += options.extra_variables; //extra variables added 1/8/2013
    //From 7/11/2014 since long URLs cause me such problems for my own web server, I'll only limit them to 2083 in IE and more in other browsers
    var isie = (navigator.userAgent.indexOf('MSIE') !== -1 || navigator.appVersion.indexOf('Trident/') > 0); //is this ie
    var maxurl = isie ? 2048 : 20000; //http://stackoverflow.com/questions/417142/what-is-the-maximum-length-of-a-url-in-different-browsers
    if (options.debug >= 5) maxurl = 0; //force the browser to do a POST
    if (options.debug >= 3) alert ("About to " + (geturl.length >= maxurl && window.postMessage ? "POST" : "GET") + " " + geturl.length + " characters: " + geturl);
    ...
    if (geturl.length >= maxurl && window.postMessage) { //window.postMessage is only supported on newer browsers
        ...
        if (!myframe) { //make the iframe
            ...
            if (options.debug >= 4) {myframe.style.width = "500px"; myframe.style.height = "100px"; myframe.style.background = "yellow";} //show the iframe 7/11/2014
        ...
        params.input = csinput; //so that it does not get double encoded, as it is going into a form and has already been encoded 6/11/2014
        if (myform) {for (var p in params) myform.elements[p].value = params[p];} //set the parameters
            ...
            for (var p in params) {
                ...
                hiddenField.setAttribute ("type", options.debug >= 4 ? "text" : "hidden"); //show the form elements
...
function CleverscriptCallBack (data, botnum) {
    ...
    ////////////////// get the TTS ////////////////// 
    if (options.avatar && options.tts_voice) { //no longer returned by 4D from early 2015, noticed 20/3/2015, fixed 25/3/2015
        CSBOTS[botnum].tts_url = 'https://www.existor.com/csapi.tts?voice=' + options.tts_voice + '&sentence=' + encodeURIComponent (data.output); //set the TTS URL
        CleverscriptPlayTTS (botnum); //play the TTS
    }
...
function CleverscriptReceiveMessage (event) {
    if (!event || !event.origin || !event.data) return; //no event
        var servermatch=0; for (var i=0; i<CSBOTS.length; i++) if (CSBOTS[i].server.indexOf (event.origin) == 0) servermatch++; //added 6/11/2014 for self-hosted bots
        if (servermatch==0 && event.origin != 'http://api.cleverscript.com' && event.origin != 'http://testapi.cleverscript.com') return; //not from API
    //if (event.origin != 'http://api.cleverscript.com' && event.origin != 'http://testapi.cleverscript.com') return; //not from API
        //CleverscriptShowObject ('Event object', event, 1);
    eval (event.data); //eval the data which will call the callback function
}

//Added 25/3/2015
function CleverscriptPlayTTS (botnum) {
    if (!botnum) botnum = 0;
    var options = CSBOTS[botnum];
    if (!options.tts_url) return; //get the URL to play
    var el = document.getElementById ('csavatarswf' + botnum);
    if (el.setSpeech) {el.setSpeech (options.tts_url); CSBOTS[botnum].tts_url = '';} //play the speech when ready
}

//This is called when the Flash finishes loading, loop through the bots and play any waiting TTS files
function startAvatar() {for (var botnum in CSBOTS) CleverscriptPlayTTS (botnum);} //play any waiting TTS
